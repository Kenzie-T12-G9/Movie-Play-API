...
import {
  addEpisodesInSeries,
  addEpisodesInSeriesValueless,
  createSerie,
  createSerieInvalidYear,
  createSerieValueless,
  createUserADM,
  createUserNotAdm,
  loginUserAdm,
  updateSerie,
...
describe('/series', () => {
  ...
  test('GET /series/:id - It should not be possible to search for a series that does not exist', async () => {
    const user = await request(app)
      .get(`/series/36d48f7f-73bd-46e2-80d8-acdc6a1f2eb0`)
      .send();
  ...
  test('GET /series/:id - It should not be possible to search for a series with an id that is not UUID', async () => {
    const user = await request(app)
      .get(`/series/36d48f7f-73bd-46e2-80d8-acdc6a1f2eb`)
      .send();
  ...
  test('POST /series/:id/episodes - It should not be possible to add an episode with an id that is not a UUID', async () => {
    const user = await request(app)
      .post(`/series/36d48f7f-73bd-46e2-80d8-acdc6a1f2eb/episodes`)
      .send(addEpisodesInSeries)
      .set('Authorization', `Bearer ${tokenADM}`);
  ...
  test('POST /series/:id/episodes - Add episode with admin user', async () => {
    ...
    expect(user.body).toHaveProperty('description');
    expect(user.body).toHaveProperty('series');
    expect(user.body.series).toHaveProperty('id');
    expect(user.body.series).toHaveProperty('name');
    expect(user.body.series).toHaveProperty('year');
    expect(user.body.series).toHaveProperty('description');
    expect(user.body.series).toHaveProperty('direction');
  ...
  test('POST /series/:id/episodes - Must not be possible to add with empty properties', async () => {
    const user = await request(app)
      .post(`/series/${idSerie}/episodes`)
      .send(addEpisodesInSeriesValueless)
      .set('Authorization', `Bearer ${tokenADM}`);
  ...
  test('POST /series/:id/episodes - Must not be possible to add, in a non-existent series', async () => {
    const user = await request(app)
      .post(`/series/36d48f7f-73bd-46e2-80d8-acdc6a1f2eb0/episodes`)
      .send(addEpisodesInSeries)
      .set('Authorization', `Bearer ${tokenADM}`);
  ...
  test('PATCH /episodes/:id - It should be possible to update the episode', async () => {
    ...
    expect(user.body.description).toEqual(
      'A chicara quebrou, procurando outra'
    );
  });

  test('PATCH /episodes/:id - It should not be possible to update an episode that does not exist', async () => {
    const user = await request(app)
      .patch(`/episodes/36d48f7f-73bd-46e2-80d8-acdc6a1f2eb0`)
      .send(updateEpisode)
      .set('Authorization', `Bearer ${tokenADM}`);
  ...
  test('DELETE /episodes/:id - It should not be possible to delete an episode that does not exist', async () => {
    const user = await request(app)
      .delete(`/episodes/36d48f7f-73bd-46e2-80d8-acdc6a1f2eb0`)
      .send()
      .set('Authorization', `Bearer ${tokenADM}`);
  ...
  test('GET /series - List all series', async () => {
    const user = await request(app).get('/series');
